...
### Opening the project through XAMPP

Place the repository at `C:\\xampp\\htdocs\\mypro_website_php`, set `APP_URL=http://localhost/mypro_website_php` in `.env`, enable Apache's `mod_rewrite`, and ensure `AllowOverride All` applies to `C:\\xampp\\htdocs`. Then open `http://localhost/mypro_website_php/`. The root `.htaccess` securely routes requests into `public/`, disables directory indexes, and keeps application source out of direct web access. Assets, links, forms, and redirects automatically retain the `/mypro_website_php` prefix.

If Apache still displays a directory listing, its override configuration is disabled. In `apache/conf/httpd.conf`, locate the `<Directory "C:/xampp/htdocs">` block, change `AllowOverride None` to `AllowOverride All`, ensure `LoadModule rewrite_module modules/mod_rewrite.so` is uncommented, and restart Apache. As a quick diagnostic, `http://localhost/mypro_website_php/public/` should load the entry point, but enabling the root rewrite is the supported setup.
